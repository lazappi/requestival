...
the queries from the informatino we have though.
...
sort them by the album release data and return the first one. If not we try a 
simpler search with just the song and artist, and again return the earliest 
result.
...
get_id <- function(Song, Artist, Release, ...) {
    ...
    results <- search_spotify(
        ...
        arrange(album.release_date_precision, album.release_date)
...
duration (in ms), whether it is explicit or not and the Spotify popularity
score.
